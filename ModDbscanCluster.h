...
#include <edm4eic/DBSCANClusterCollection.h>
...
namespace eicrecon {
    ...
    using ModDbscanClusterAlgorithm = algorithms::Algorithm<
        ...
        algorithms::Output<edm4eic::DBSCANClusterCollection>
    >;
    ...
    public:
        void process(const Input& input, const Output& output) const final {
            ...
            auto [dbscan_clusters] = output;  // Get the output clusters
            ...
            // Create and store the clusters in output (dbscan_clusters)
            for (const auto& cluster : clusters) {
                // Construct a new cluster and add it to the output collection
                edm4eic::DBSCANCluster new_cluster;
                for (const auto& point : cluster) {
                    // Convert each (x, y, z, energy) point into your desired output format
                    // Assuming your cluster class has addHit method
                    new_cluster.addHit(point);
                }
                dbscan_clusters->push_back(new_cluster);  // Add the constructed cluster to the output collection
            }
        }
    ...
    public:
        ...
        std::unordered_map<int, std::vector<std::tuple<float, float, float, float>>> cluster(const std::vector<std::tuple<float, float, float, float>>& points) {
    ...
    private:
        ...
        // Check if a point should be considered as noise based on energy threshold
        ...
        // Range query to find neighbors within the epsilon distance
        std::vector<size_t> range_query(const std::vector<std::tuple<float, float, float, float>>& points, size_t idx) const {
            ...
            for (size_t i = 0; i < points.size(); ++i) {
                if (i == idx) continue;  // Skip the point itself
        ...
        double distance(const std::tuple<float, float, float, float>& p1, const std::tuple<float, float, float, float>& p2) const {
            ...
            return std::sqrt(dx * dx + dy * dy + dz * dz);  // No energy in distance calculation
        }
        ...
        // Split the clustered points into separate clusters
